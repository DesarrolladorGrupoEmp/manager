$(function(){
	...
	*/
	...
	$('#salario_mask').mask('000.000.000.000.000', {reverse: true});
            ...
            $.ajax({
                ...
                success: function(data){
                	...
                	for(i=0; i<7; i++){
                		...
                		cadena = "fkID_contrato="+idContrato+"&fkID_documento="+objeto[i]["contador"]+"&fecha="+fecha+"&ruta="+objeto[i]["nombre"];
	        		...
	        		$.ajax({
	          			url: "../controller/ajaxController12.php",
	          			data: "fkID_empleado="+empleado+"&fkID_documento="+i+"&fecha="+fecha+"&tipo=inserta&nom_tabla=archivos_empleado",
	        		})
	        		.done(function(data){
    ...
    function registraArchivo(cadena){
	    ...
	    $.ajax({
	    	url: '../controller/ajaxController12.php',
	    	data: cadena+"&tipo=inserta&nom_tabla=archivos_empleado",
	    })
	    .done(function(data) {            
	        console.log(data);
	    })
	...
	function cargaDoc(idContrato,tipo){
		...
		var query_archivos = "select *,archivos_empleado.pkID as ID FROM `archivos_empleado` LEFT JOIN documentos_empleado ON documentos_empleado.pkID = archivos_empleado.fkID_documento WHERE fkID_contrato ="+idContrato+" and fkID_documento="+tipo;
	    	...
	    	if(data.estado != "Error"){
		    	...
		    	for(var i = 0; i < data.mensaje.length; i++){
		    		if(data.mensaje[i].ruta){
			    		$("#archivos_res"+tipo).append(	
			    		'<div class="form-group" id="frm_group'+data.mensaje[i].ID+'">'+		                		               
			            ...
			            '<div class="form-group">'+
		        			...
		        			'<p style="width: 90%;display: inline;" > '+data.mensaje[i].fecha+'</p> <br>'+
	...
	function elimina_documento(id_documento,tipo){
	    ...
	    if(confirma == true){
	      ...
	      $.ajax({
	            url: '../controller/ajaxController12.php',
	            data: "pkID="+id_documento+"&tipo=eliminar&nom_tabla=archivos_empleado",
	        })
	        .done(function(data) {            
	            //---------------------
	            console.log(data);
