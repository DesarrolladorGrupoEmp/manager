$(function(){
	...
	function elimina_hvida(id_hvida){
	    ...
	    if(confirma == true){
	      ...
	      $.ajax({
	            url: '../controller/ajaxController12.php',
	            data: "pkID="+id_hvida+"&tipo=eliminar&nom_tabla=hoja_vida",
	        })
	        .done(function(data) {            
	            //---------------------
	            console.log(data);
	...
	function carga_estudios(id){
		...
		var query_estudios = "SELECT hoja_vida.pkID as pkID_hojaVida, estudio.*, tipo_estudio.nombre as nom_tipoEstudio, hoja_estudio.pkID as pkID_regHojaEstudio"+ 
	...
	function selectEstudio(id,nombre,tipo){
		...
		if(id!=""){
			...
			if(document.getElementById("pkID_estudio_"+id)){
				...
				$("#frm_estudios_hvida").append(
					'<div class="form-group" id="frm_group'+id+'">'+
		                '<label for="pkID_estudio_'+id+'" class="control-label" style="margin-right: 20px;">'+tipo+'</label>'+		                
		                '<input type="text" style="width: 90%;display: inline;" class="form-control" id="pkID_estudio_'+id+'" name="pkID_estudio" value="'+nombre+'" readonly="true"> <button name="btn_actionRmEstudio" data-id-estudio="'+id+'" data-id-frm-group="frm_group'+id+'" type="button" class="btn btn-danger"><span class="fa fa-remove"></span></button>'+		                
		            '</div>'
		            );
	...
	function selectEstudioBusqueda(id,nombre,tipo){
		...
		if(id!=""){
			...
			if(document.getElementById("pkID_estudio_busqueda_"+id)){
				...
				$("#frm_estudios_busquedaHvida").append(
					'<div class="form-group" id="frm_group'+id+'">'+
		                '<label for="pkID_estudio_busqueda_'+id+'" class="control-label" style="margin-right: 20px;">'+tipo+'</label>'+		                
		                '<input type="text" style="width: 90%;display: inline;" class="form-control" id="pkID_estudio_busqueda_'+id+'" name="pkID_estudio" value="'+nombre+'" readonly="true"> <button name="btn_actionRmEstudioBusqueda" data-id-estudio="'+id+'" data-id-frm-group="frm_group'+id+'" type="button" class="btn btn-danger"><span class="fa fa-remove"></span></button>'+		                
		            '</div>'
		            );
		...
		if(id!=""){
			...
			if(document.getElementById("pkID_estudio_"+id)){
				...
				$("#frm_estudios_hvida").append(
					'<div class="form-group" id="frm_group'+id+'">'+
		                '<label for="pkID_estudio_'+id+'" class="control-label" style="margin-right: 20px;">'+tipo+'</label>'+		                
		                '<input type="text" style="width: 90%;display: inline;" class="form-control" id="pkID_estudio_'+id+'" name="pkID_estudio" value="'+nombre+'" readonly="true"> <button name="btn_eliminaEstudio" data-id-estudio="'+id+'" data-num-reg="'+numReg+'" data-id-frm-group="frm_group'+id+'" type="button" class="btn btn-danger"><span class="fa fa-remove"></span></button>'+		                
		            '</div>'
		            );
	    ...
	    .done(function(data) {
	    	...
	    	if(data.estado != "Error"){
		    	...
		    	for(var i = 0; i < data.mensaje.length; i++){
			    	...
			    	$("#archivos_res").append(	
			    		'<div class="form-group" id="frm_group'+data.mensaje[i].pkID+'">'+		                		               
			            ...
			            '<div class="form-group">'+
		        			...
		        			'<p style="width: 90%;display: inline;" > '+data.mensaje[i].des_archivo+'</p> <br>'+
	...
	*/
	...
	function carga_tecnico(){
		...
		var consulta_empresa = "select estudio.*, tipo_estudio.nombre as nom_tipo_estudio"+ 
								" FROM `estudio`"+
								" INNER JOIN tipo_estudio ON tipo_estudio.pkID = estudio.fkID_tipoEstudio"+
